...
const Feed = () => { 
    ...
    useEffect(() => {
        ...
        return () => { db1.off('value', getMyFeed); };
    }, [])
    ...
    useEffect(() => { 
        ...
        return () => {friendDB.forEach(db => db.off('value', getFriendFeed))}
    }, [])
    ...
    const makeContent = () => { 
        return (
            feed.map(content => {
                ...
                return (<div className={classes.strip}>
    ...
    return (<header className="App-header" style={{justifyContent: "flex-start"}}>
